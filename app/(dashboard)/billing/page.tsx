...
const TIERS = [
  {
    ...
    price: "$0",
    sends: "5,000",
    features: ["Community Support", "API Access", "Standard Latency", "1 Webhook Endpoint"],
  },
  {
    level: 1,
    name: "Growth",
    price: "$49",
    sends: "50,000",
    features: ["Email Support", "Advanced Analytics", "Reduced Latency", "5 Webhook Endpoints"],
  },
  {
    level: 2,
    name: "Scale",
    price: "$199",
    sends: "250,000",
    features: ["Priority Support", "SLA Guarantee", "Ultra-low Latency", "Unlimited Webhooks"],
  },
];
    ...
    <div className="space-y-10 max-w-6xl">
            ...
            return (
              <div 
                ...
                className={`relative bg-card rounded-2xl p-6 flex flex-col border ${
                  isCurrent ? "border-teal shadow-[0_0_30px_rgba(0,200,150,0.1)]" : "border-border"
                }`}
              >
                {isCurrent && (
                ...
                <div className="mb-4">
                  ...
                  <div className="mt-2 flex items-baseline text-3xl font-bold text-white">
                    {tier.price}
                    <span className="ml-1 text-sm font-medium text-text-muted">/mo</span>
                  </div>
                </div>

                <div className="mb-6 pb-6 border-b border-border">
                  <span className="text-sm text-text-primary"><span className="font-bold text-white">{tier.sends}</span> Notifications</span>
                </div>

                <ul className="flex-1 space-y-4 mb-8">
                  {tier.features.map((feature, i) => (
                    <li key={i} className="flex items-start gap-3">
                      <svg className="h-5 w-5 text-teal shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div className="bg-navy border border-border-2 rounded-xl p-8 text-center mt-12">
        ...
        <p className="text-sm text-text-muted mb-4 max-w-lg mx-auto">
          Contact us for custom enterprise plans with massive notification volumes, dedicated IP addresses, and white-glove onboarding.
        </p>
