  ...
  return (
    <div className="max-w-3xl space-y-8">
      <div>
        ...
        <p className="text-sm text-text-muted mt-1">Configure your protocol identity and sender preferences.</p>
      </div>
        ...
        <form onSubmit={handleSave} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-1.5">
              <label className="text-sm font-medium text-text-secondary">Protocol ID</label>
              <Input 
                value={profile.id}
                disabled
                className="w-full bg-card-2 text-text-muted cursor-not-allowed font-mono text-xs"
              />
              <p className="text-xs text-text-muted mt-1">Immutable unique identifier</p>
            </div>
            ...
            <div className="space-y-1.5">
              <label className="text-sm font-medium text-text-secondary">Brand Logo URL</label>
              <Input 
                type="url"
          ...
          <div className="space-y-1.5 max-w-md">
            ...
            <Input 
              ...
              onChange={(e) => setProfile(p => ({ ...p, fromName: e.target.value }))}
              placeholder="e.g. Herald Updates"
              className="w-full"
            />
            <p className="text-xs text-text-muted mt-1">This appears as the sender name in email bridging.</p>
          </div>
          ...
          <div className="pt-6 flex items-center gap-4 border-t border-border">
            ...
            {saved && <span className="text-sm text-green flex items-center gap-1.5 animate-in fade-in duration-300">
      ...
      <div className="bg-red/10 border border-red/30 rounded-xl p-6">
        ...
        <p className="text-red/80 text-xs max-w-xl mb-4">
          Deleting your project will permanently drop all notification queues, revoke all active API keys, and terminate your webhook streams.
        </p>
